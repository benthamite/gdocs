...
The diff engine emits per-paragraph ~createParagraphBullets~ requests,
which always produce flat (level-0) lists.  The full conversion path
has a separate mechanism (~gdocs-convert--fixup-list-nesting~) that
groups bullets and inserts leading tabs so that the API infers correct
nesting levels, but the diff path cannot use that mechanism because it
interleaves modifications with insertions at varying document
positions.
...
5. For each such group, emit four kinds of requests in order:
   ...
   - ~insertText~ to insert /N/ tab characters at each nested
     ...
     positions.
   - A single ~createParagraphBullets~ spanning the entire group.
     The API counts the leading tabs, assigns the correct nesting
     levels, and removes the tabs afterward.
...
paragraph produces correct operations; the word-level approach never
falls back to a bulk replacement.
...
hook on ~org-mode-hook~).  You can also toggle it manually.
