...
4. For each destructive operation (element deletion or cross-type
   ...
   plain text.
5. Prompt the user for each at-risk operation with ~y-or-n-p~, showing
   the element text, comment author, and comment content.
6. Remove declined operations from the diff operation list.
